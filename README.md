...
### Local Mailchimp Configuration
Form blocks are generated by the Mailchimp module. If the module isn't able to access our Mailchimp API key, these forms will fail to be generated. To configure Mailchimp on your local environment, create `settings.local.php` in `web/sites/default` and add the Mailchimp API key to `$config['mailchimp.settings']['api_key']` in that settings file. This file is ignored by default, but be sure you do not accidentally commit and expose our API key publicly.
